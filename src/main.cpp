...
void buildLineVertices() {
    ...
    for (int x = 0; x < N; ++x) {
        for (int y = 0; y < N; ++y) {
            ...
            if (x < N - 1) {  // Vertical grid lines (not at the last column)
            ...
            if (y < N - 1) {  // Horizontal grid lines (not at the last row)
...
void buildDensityVertices(const std::vector<float> &d){
    ...
    for (int x = 0; x < N; ++x)
    {
        for (int y = 0; y < N; ++y)
        {
            ...
            std::cout << "Clamped density: " << val << std::endl;
...
void advect(std::vector<float> &d, std::vector<float> &d0, glm::vec2 velocity[N][N], float dt) 
{
    ...
    // std::cout << "Density after advecting" << std::endl;
    // for (float x : density) {
    //     std::cout << x << " ";
    // }
    // std::cout << std::endl;
